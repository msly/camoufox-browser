 ...
 *
 * This is largely compatible with agent-browser's snapshot/ref workflow:
 * - `snapshot -i` returns a tree with `[ref=e1]` markers.
...
> {
  ...
  const fn = new Function("return " + scriptBody)() as (rootSel: string) => unknown;
  return page.evaluate(fn, rootSelector) as any;
}
...
export async function getEnhancedSnapshot(
  ...
  const ariaTree = await (locator as Locator).ariaSnapshot();
...
function processLine(
  ...
  if (shouldHaveRef) {
    const ref = nextRef();
    const resolvedName = isInteractive ? (name ?? "") : (name as string);
    const nth = tracker.getNextIndex(roleLower, resolvedName);
    tracker.trackRef(roleLower, resolvedName, ref);
